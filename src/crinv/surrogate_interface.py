...
class ForwardSurrogate(Protocol):
    """Forward surrogate interface (frozen during inverse).

    Input is binary structure (or STE proxy): [B,128,128] float tensor in [0,1].
    Output is RGGB spectra: [B,2,2,C].
    """
...
class MockSurrogate:
    ...
    def predict(self, x_binary: torch.Tensor) -> torch.Tensor:
        ...
        base = torch.stack([q00, q01, q10, q11], dim=-1).view(B, 2, 2)  # [B,2,2]

        # Expand into spectra channels deterministically.
        C = self.n_channels
        out = base[..., None].repeat(1, 1, 1, C)
        # Add a small per-channel slope to avoid pathological flatness.
        slope = torch.linspace(0.0, 0.01, steps=C, device=x_binary.device, dtype=x_binary.dtype)
        out = out + slope.view(1, 1, 1, C)
        return out


@dataclass
class CRReconSurrogate:
    """Load and run CR_recon forward model checkpoints.

    This integrates the existing CR_recon project (external folder) by importing its model
    registry dynamically from `forward_model_root`.
    """
    ...
    def __post_init__(self) -> None:
        ...
        # Import CR_recon's model registry from the external root.
        import importlib
        import sys
        ...
        try:
            ...
            # Keep sys.path entry for downstream imports (model class definitions) during runtime.
            # Removing it can break torch's lazy module resolution. We keep it intentionally.
            pass
        ...
        # Input normalization matches CR_recon dataset config.
        self._map_to_pm1 = bool(cfg.get("data", {}).get("map_to_pm1", True))

    @torch.no_grad()
    def predict(self, x_binary: torch.Tensor) -> torch.Tensor:
        """x_binary: [B,128,128] float in [0,1] (or STE proxy)."""
        if x_binary.ndim != 3:
            raise ValueError(f"x_binary must be [B,H,W], got {tuple(x_binary.shape)}")
        x = x_binary.to(device=self.device, dtype=torch.float32)
        # Model expects (B,1,128,128). Training data maps struct to [-1,1].
        if self._map_to_pm1:
            x = (x * 2.0) - 1.0
        x = x.unsqueeze(1)
        y = self._model(x)
        # Expected shape: [B,2,2,C]
        return y
