...
def _rectangles_from_binary_grid(grid: np.ndarray) -> list[tuple[int, int, int, int]]:
    ...
    Returns rectangles as (x0, y0, x1, y1) in pixel coordinates where x in [0,W], y in [0,H].

    Strategy:
    - Find horizontal runs of 1s per row -> segments [x0,x1)
    - Merge identical segments vertically into maximal rectangles
    """
    if grid.ndim != 2:
    ...
    # Row segments: for each y, list of (x0,x1).
    ...
    active: dict[tuple[int, int], tuple[int, int]] = {}  # (x0,x1) -> (y0, y1_exclusive)

    for y in range(h + 1):
        segs = segments_per_row[y] if y < h else []
        seg_set = set(segs)

        # Close rectangles whose segment doesn't continue.
        ...
        # Extend or start new rectangles for segments present.
        for seg in segs:
            if seg in active:
                y0, _y1 = active[seg]
                active[seg] = (y0, y + 1)
...
def export_struct128_to_gds(
    ...
    *,
    ...
    Naming/layer rules (blueprint):
    - Filename: structure_{id:05d}.gds (recommended)
    - Cellname: structure_{id:05d}
    - Layer: 1 (datatype 0)
    """
    ...
    for x0, y0, x1, y1 in rects:
        # Coordinate system: x right, y up. Map pixel row y -> y coordinate.
        # Keep it simple: y increases with row index.
        r = gdstk.rectangle(
            (x0 * ps, y0 * ps),
            (x1 * ps, y1 * ps),
    ...
    # Work around by writing to an ASCII-only temp path then copying bytes to destination.
